	...
	for x in range(halfHours):
		...
		for elem in features:
			...
			if (indices[idx]//7) == x:
				...
				print x
				print
				concatData[x][concatentry] = elem[0]
				concatData[x][concatentry+7] = elem[1]
				concatData[x][concatentry+14] = elem[2]
				concatData[x][concatentry+21] = elem[3]
				concatData[x][concatentry+28] = elem[4]
				if elem[5] > 0 :
					concatData[x][concatentry+35] = 1
				if elem[6] > 0 :
					concatData[x][concatentry+42] = 1
				if elem[7] > 0 :
					concatData[x][concatentry+49] = 1
				if elem[8] > 0 :
					concatData[x][concatentry+56] = 1
